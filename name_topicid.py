...
# 转换name到topicid
with open('./res_container/res10', 'r') as f0:
    line = f0.readline()
    # full = {}
    # full_relation=[]
    ...
    while line:
        ...
        if len(new_topic_dic)!= 0:
            ...
            with open('.recommend_container/recommend5/recommend_list_prior', 'a') as f1:
                f1.write(data+'\n')

        # 写统计结果
        data_count = js.dumps(count_topic_dic, ensure_ascii=False)
        with open('.recommend_container/recommend5/recommend_count', 'a') as f2:
            f2.write(data_count+'\n')

        # 写未转换topic的名单
        if len(lose_topic_id)!=0:
            data3 = js.dumps(lose_topic_id, ensure_ascii=False)
            with open('.recommend_container/recommend5/recommend_lose', 'a') as f3:
                f3.write(data3+'\n')
        # 循环下一行
        line=f0.readline()
...
with open('.recommend_container/recommend5/recommend_count', 'a') as ff:
    ff.write(data_all+'\n')
    ff.write(data_id+'\n')
